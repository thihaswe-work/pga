...
import {
  DeleteDialog,
  OpenDialog,
} from "@/features/job-applied-forms/components/dialog";
import { getColumns } from "@/features/job-applied-forms/components/columns";
import { JobApplyForm } from "@/types/api";
import { useState } from "react";
import { useNavigate } from "react-router";
import { toast } from "sonner";

export default function Page() {
  const [selectedDetail, setSelectedDetail] = useState<JobApplyForm | null>(
    null
  );
  ...
  const handleViewClick = (detail: JobApplyForm) => {
    setSelectedDetail(detail);
    setDialogOpen(true);
  };
  const handleViewDelete = (detail: JobApplyForm) => {
    setSelectedDetail(detail);
